...
async def send_online_update(bot, channelid : int, username : str, habbo_data: dict ):
    ...
    if status == False:
        ...
        online_since = habbo_data.get('lastAcccessTime') 
    ...
    embed = discord.Embed(title=f'User {a}', description= description, color = c)
    embed.set_footer(icon_url= bot.user.avatar.url, text= f'{bot.user.name} . {datetime.utcnow()}')

    await chan.send(embed=embed, content='<@&1435727697048371312>')
